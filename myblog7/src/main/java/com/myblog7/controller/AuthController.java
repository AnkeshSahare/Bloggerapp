...
//    }
    ...
    public ResponseEntity<?> registerUser(@RequestBody SignUpDto signUpDto){
        ...
        if(emailExist){
            return new ResponseEntity<>("Email id Already Exist", HttpStatus.BAD_REQUEST);//400
        }

        Boolean usernameExist = userRepository.existsByUsername(signUpDto.getUsername());
        if(usernameExist){
            return new ResponseEntity<>("Username is Already Exist", HttpStatus.BAD_REQUEST);//500
        }
